...
pub fn add_image(path: PathBuf, category: String) -> Result<i64, String> {
    ...
    match stmt.query_row([relative_path, &category, filename], |row| {
        row.get::<_, i64>(0)
    }) {
        Ok(id) => Ok(id),
        Err(_) => {
...
pub fn search_images_by_tags(tags: Vec<String>) -> Result<Vec<Image>, String> {
    ...
    let mut params: Vec<Box<dyn ToSql>> = tags
        .iter()
...
pub fn get_batch_image_ids(
    file_paths: &[(PathBuf, String)],
) -> Result<HashMap<PathBuf, i64>, String> {
    let conn = connect_db()?;
    ...
    for (path, category) in file_paths {
        ...
        if let Ok(id) = stmt.query_row([relative_path, category, filename], |row| {
            row.get::<_, i64>(0)
        }) {
...
pub fn get_batch_image_tags(
    image_ids: &HashMap<PathBuf, i64>,
) -> Result<HashMap<i64, Vec<TagInfo>>, String> {
    let conn = connect_db()?;
    let mut result = HashMap::new();
