...
export default function LockerPage() {
    ...
    const handleFileDrop = useCallback(
        async (droppedFiles?: string[]) => {
            ...
            if (droppedFiles?.length > 0) {
                ...
                try {
                    const selectedFiles = await open({
                        multiple: true,
                        directory: false,
                        filters: [{ name: "Images", extensions: ["png", "jpg", "jpeg", "jfif", "webp"] }],
                    })
            ...
            if (!validFiles.length) {
                toast({ title: "Invalid Files", description: "No valid image files were selected.", variant: "destructive" })
                return
            }
            ...
            if (duplicateFiles.length === validFiles.length) {
                toast({ title: "Duplicate Files", description: "All selected files already exist in this category.", variant: "destructive" })
                return
            }
            ...
            for (const file of validFiles) {
                try {
                    ...
                    if (file instanceof globalThis.File) {
                        ...
                        response = await invoke<FileMoveResponse>("move_file", { originalPath: file, category })
                    }
    ...
    const handleDelete = async (file: File) => {
        try {
            await invoke("delete_file", { category: file.category, name: file.name })
            ...
            toast({ title: t("toast.titleType.success"), description: `${file.name} deleted successfully` })
        } catch (error) {
            toast({ title: t("toast.titleType.error"), description: `Failed to delete Image ${file.name}`, variant: "destructive" })
        }
    ...
    const handleMove = async (newCategory: string) => {
        ...
        try {
            await invoke("move_file_category", { oldCategory: selectedFile.category, newCategory, fileName: selectedFile.name })
            setFiles((prevFiles) =>
                prevFiles.filter((f) => f.name !== selectedFile.name || f.category !== selectedFile.category),
            )
            toast({ title: t("toast.titleType.success"), description: `File ${selectedFile.name} moved to ${newCategory} successfully` })
        } catch (error) {
            toast({ title: t("toast.titleType.error"), description: "Failed to move file", variant: "destructive" })
        }
    ...
    const onCategoryChange = useCallback(
        async (value: string) => {
            ...
            try {
                const data: FileResponse = await invoke("get_files", { page: 1, limit: -1, category: value })
                setAllFiles(data.files)
                const startIndex = 0
                const endIndex = Math.min(imagesPerPage, data.files.length)
                setFiles(data.files.slice(startIndex, endIndex))
                ...
                setFiles([])
                setTotalPages(0)
                toast({ title: t("toast.titleType.error"), description: "Failed to fetch files for category", variant: "destructive" })
            } finally {
                setIsLoading(false)
            }
        },
        ...
        const loadInitialData = async () => {
            try {
                ...
                if (rememberCategory) {
                    ...
                    if (lastCategory) initialCategory = lastCategory
                }
    ...
    const confirmBulkDelete = async () => {
        try {
            await Promise.all(
                selectedFilesForBulkAction.map((file) => invoke("delete_file", { category: file.category, name: file.name }))
            )
            ...
            toast({ title: t("toast.titleType.success"), description: t("locker.bulk.deleteSuccess", { count: selectedFilesForBulkAction.length }) })
            fileGridRef.current?.clearSelection()
        } catch (error) {
            toast({ title: t("toast.titleType.error"), description: t("locker.bulk.deleteError"), variant: "destructive" })
        }
    ...
    const handleBulkMoveConfirm = async (newCategory: string) => {
        try {
            await Promise.all(
                selectedFilesForBulkAction.map((file) =>
                    invoke("move_file_category", { oldCategory: file.category, newCategory, fileName: file.name })
                )
            )
            ...
            toast({ title: t("toast.titleType.success"), description: t("locker.bulk.moveSuccess", { count: selectedFilesForBulkAction.length, category: newCategory }) })
            await fetchAllFiles()
            fileGridRef.current?.clearSelection()
        } catch (error) {
            toast({ title: t("toast.titleType.error"), description: t("locker.bulk.moveError"), variant: "destructive" })
        }
                            ...
                            <div className="container mx-auto max-w-[2000px]">
                                ...
                                {isLoading ? (
                                    ...
                                    <FileGrid
                                        ref={fileGridRef}
                                        allFiles={allFiles}
                                        onViewFileAction={async (file) => { await show_in_folder(file.filepath) }}
                                        onDeleteFileAction={(files) => {
                                            if (Array.isArray(files)) { setSelectedFilesForBulkAction(files); setBulkDeleteDialogOpen(true) }
                                            else { setSelectedFile(files); setDeleteDialogOpen(true) }
                                        }}
                                        onMoveFileAction={(files) => {
                                            if (Array.isArray(files)) { setSelectedFilesForBulkAction(files); setBulkMoveDialogOpen(true) }
                                            else { setSelectedFile(files); setMoveDialogOpen(true) }
                                        }}
                                        onTagAction={(files) => {
                                            if (Array.isArray(files)) { setSelectedFilesForBulkAction(files); setBulkTagDialogOpen(true) }
                                            else { setSelectedFileForTags(files); setTagManagerOpen(true) }
                                        }}
                                ...
                                {selectedFilesForBulkAction.length > 0 && (
                                    <BulkTagManagerDialog
                                        ...
                                        onClose={async () => { setBulkTagDialogOpen(false); fileGridRef.current?.clearSelection(); await fetchAllFiles() }}
                                    />
                                )}
                                <PaginationControls
                                    currentPage={currentPage}
